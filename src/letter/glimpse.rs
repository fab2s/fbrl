...
fn make_grid(
    ...
    // Stack [gx, gy] -> [patch_h, patch_w, 2] via reshape+cat.
    let gx_flat = gx.reshape(&[patch_h * patch_w, 1])?;
    let gy_flat = gy.reshape(&[patch_h * patch_w, 1])?;
    let stacked = gx_flat.cat(&gy_flat, 1)?; // [patch_h*patch_w, 2]
    let base_grid = stacked.reshape(&[1, patch_h, patch_w, 2])?;
