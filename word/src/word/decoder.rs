//! VisualDecoder — reconstructs 128×256 word images from readout latents.
//!
//! Input: flattened readout states [B, n_positions * latent_dim] (e.g. 1024).
//! Output: [B, 1, 128, 256] reconstructed word image.

use flodl::autograd::Variable;
use flodl::nn::{
    BatchNorm2d, Buffer, Conv2d, ConvTranspose2d, Linear, Module, Parameter,
};
...
impl WordDecoder {
    /// Create a decoder for word-sized output.
    ///
    /// input_dim: n_positions * latent_dim (e.g. 4 * 256 = 1024).
    /// output: [B, 1, 128, 256].
    pub fn new(input_dim: i64, output_h: i64, output_w: i64) -> Result<Self> {
        let spatial_h = output_h / 4;  // 32
        let spatial_w = output_w / 4;  // 64
...
impl Module for WordDecoder {
    fn name(&self) -> &str { "word_decoder" }
